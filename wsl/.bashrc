...
__conda_setup="$('/home/stanfish/anaconda3/bin/conda' 'shell.bash' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "/home/stanfish/anaconda3/etc/profile.d/conda.sh" ]; then
        . "/home/stanfish/anaconda3/etc/profile.d/conda.sh"
    else
        export PATH="/home/stanfish/anaconda3/bin:$PATH"
    fi
fi
unset __conda_setup
# <<< conda initialize <<<
...
# completion for globus
if type globus > /dev/null 2>&1; then
    eval "$(globus --bash-completer)"
fi

# bin installed by rust
export PATH="/home/stanfish/.cargo/bin:$PATH"
# lib path
...
case ":$PATH:" in
    *:/home/stanfish/.juliaup/bin:*)
        ;;

    *)
        export PATH=/home/stanfish/.juliaup/bin${PATH:+:${PATH}}
        ;;
esac
...
# activate mise
eval "$(/home/stanfish/.cargo/bin/mise activate bash)"
...
__gh_handle_standard_completion_case() {
    ...
    if ((${#COMPREPLY[*]} == 1)); then
        ...
        __gh_debug "Removed description from single completion, which is now: ${COMPREPLY[0]}"
    else
